...
> **この付録のゴール**  
> コマンドラインでデータや Web ページを取得・保存する万能ツール **`curl`（カール）** の基本概念・各 OS でのインストール方法・サーバー負荷を下げるスリープ（ウェイト）オプション・および本書で使うボートレースデータ取得スクリプトの全オプションの意味を丁寧に理解する。  
...
#### E.4 🛡️ 【絶対必須ルール】サーバー負荷を下げるスリープ（アクセス待ち時間）オプション
