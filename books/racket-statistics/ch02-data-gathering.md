...
> **この章のゴール**  
> Webや外部ツール（`curl`）を活用してボートレースのデータ（CSV/テキスト）を手元に取得し、プログラムで扱える状態にする。  
...
ターミナル（またはコマンドプロンプト）から、以下のコマンドを実行すると、Web上のCSVファイルやHTMLを手元のフォルダに保存できます。
